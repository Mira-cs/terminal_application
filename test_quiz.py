...
def test_quiz_progressbar():
    ...
    # Test that the bar is empty at the beginning
    assert bar.index == 0
    
    # Test that the bar advances after calling next()
    bar.next()
    assert bar.index == 1
    
    # Test that the bar is full after calling next() num_questions times
